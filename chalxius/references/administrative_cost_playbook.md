...
| Release validation setup | an early architecture or applicability failure still pays to materialize later isolated lanes | bytes and time copied before each phase; cost on intentional phase-one failure | create each canonical isolated lane only when its phase becomes eligible | every executed lane remains an exact manifest-bound copy; success still requires the complete matrix |
