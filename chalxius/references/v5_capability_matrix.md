...
| Historical runtime archive and cutover | `scripts/archive_runtime.py`, `scripts/runtime_cutover_project_validation.py`, `scripts/runtime_cutover.py`, schema-2 `runtime_content_sha256`/`historical_archive_root`, schema-1 identity registry | additive host compatibility | release CHX-027–034; current CHX-062 and CHX-067 | archive root outside skill discovery and independent of card-controlled trust; manifest-listed regular-file copy only; component-wise no-follow and canonical-path checks; traversal, links, special/cross-device/extra files and registry drift rejected; exact object and registry sealed read-only and revalidated on every terminal read; host entrypoints disable bytecode before local imports and cannot contaminate the runtime they validate; protected install and rollback require explicit project inventory, externally approved candidate manifest, exact candidate tree, and a hash-approved validation receipt binding the complete runtime diff, release matrix, prior current audit, terminal rounds, historical bindings, and exact audit-relevant project digest; a classified impacting delta performs one deep audit while building the receipt, while a non-impacting delta reuses the unchanged prior audit; cutover and postflight reuse one receipt or one explicitly forced deep audit and never duplicate semantic reconstruction; same-parent cutover and automatic restore remain mandatory; archived code is never imported or executed; no Paper, Evidence, Research, Candidate, Certification, Fact, CHX, Reader, Learner, Campaign, or L3/L4 authority |
| Release validation matrix | `scripts/release_validation.py`; `scripts/aggressive_bug_audit.py` | additive release validation | current CHX-064–065 and CHX-068; extends field CHX-014 | one approved manifest identity is copied into canonical no-symlink isolated lanes with separate temporary and runtime-archive state; compatible baseline checks may run in parallel, while the snapshot-sensitive audit is phase-barriered afterward; source and every lane receive exact pre/post snapshots; before any baseline subprocess, a contained regular-file mutant registry preflight requires exactly one occurrence of every attack target; one aggregate receipt rejects incomplete results, identity mixing, nonzero exits, timeouts, drift, stale registry plans, or barrier removal; the audit also suppresses bytecode intrinsically in every child; coordinator and audit self-mutants guard the seams; release evidence only, with no Paper, Research, Candidate, Certification, Gateway, Fact, or truth effect |
...
| Campaigns | `campaign-create`, `campaign-activate`, `campaign-status`, `campaign-target-add`, `campaign-target-archive`, `campaign-update`, `research-goal-intake`; explicit `frontier --campaign`, `plan-round --campaign` | adapt; user-approved 2026-07-30 and goal-intake repair | CHX-020, CHX-033, CHX-039; CHX-063 | optional durable multi-round objective/target/constraint/stop/history envelope; Main may pass an exact id, while Operator `auto` may compile the user's exact stated objective without Campaign jargon; lexical exact reuse or prospective creation only; returned id binds future Research; no fuzzy/`ACTIVE` selection, retagging, second scheduler, continuous advancement, closure, or Fact effect |
...
| Brave Future advisory L3/L4 | `research-goal-intake`, `brave-future-enable`, `brave-future-status`, `brave-future-audit`, `campaign-reassess`, `campaign-reassess-decide`, `brave-future-disable`; explicit `frontier --campaign ID --brave-future --view actionable|all-active|history` | additive goal-triggered BF-1 under `auto` or `deep`; BF-2/BF-3 evidence-gated | user-approved cautious recovery guide; integration `CHX-018`; CHX-063 and CHX-066 | exact Campaign internally and never `ACTIVE`; a user objective can create/exact-match that scope without naming it; `fast` retains explicit activation; current V5 four-factor frontier remains the only scheduler; typed complete repairs alone may collapse roots; BF-2/BF-3 require a real ingested-attempt blockage, dry-run is zero-write, persistence is one atomic nontruth transaction, repeat signatures park, and every decision remains advisory with no round, plan, Pulse, dispatch, Campaign-close, Candidate, Certification, Gateway, Fact, Reader, or Learner effect |
...
nontruth envelope. In 0.6.4 that id may also be compiled from the user's exact
...
The Brave Future connection introduced in 0.6.0 and retained without authority expansion through 0.6.4 is narrower still. It may read an exact
