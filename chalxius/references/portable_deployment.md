...
Schema-v2 repair capability bytes are bounded to one command-local snapshot and
64 MiB aggregate per read phase, with fresh lock-bound rechecks after every
mutation boundary. Current textual worker returns reject C0 control bytes while
historical 0.7.15 cards and binary artifacts remain readable. The entrypoint
policy disables bytecode before imports; no generated bytecode is part of a
release archive or manifest.
