...
def main() -> int:
    ...
    identity_requirements = {
        "SKILL.md": (
            "name: chalxius",
            "# Chalxius 0.7.0 — Failure-Informed Efficiency",
            ...
            "`attack-report --full`",
            "`fresh_adverse_missing`",
            "`批准建议 N`",
            "ordinary-language sentence stating what each checks",
            "unknown family stays in `--full`",
            "silence and successful return never activate a",
            "memory-add --current-assurance",
            "plan-supervision-round SOURCE_ROUND",
            "--component-id COMPONENT",
            "logical components from selected Research ancestry",
            "plan-computation-execution SOURCE_ROUND ASSIGNMENT",
            "`independent_adverse_required` remains a later exact-Candidate adverse marker",
            "Supervision validation uses only one command-local, component-bound, cycle-safe inspection context",
            "authoritatively rederives the connected components",
            "required supervisor result ids are automatically bound into the release closure",
            "an identical retry reuses that exact Research",
            "static reproduced-failure registry",
            "a plain interpretive insight is not treated as a proof",
            "New V5 Pulse planning is retired",
            "one root-space node and grants no write space",
            "bounded oral follow-up fast",
        ...
        "INHERITANCE.lock.json": (
            '"skill_name": "chalxius"',
            '"version": "0.7.0"',
            '"release_codename": "Failure-Informed Efficiency"',
            ...
            '"supervision_validation": "one_ephemeral_component_bound_cycle_safe_inspection_context_with_complete_outer_authority_and_hash_validation"',
        ...
        "references/portable_deployment.md": (
            "The 0.7.0 `Failure-Informed Efficiency` release artifact",
